...
namespace
{
    ...
    CS200::RGBA Dim(CS200::RGBA c) { return (c & 0xFFFFFF00u) | 0x80u; }
    ...
    const char* TypeNames[] = { "Floor", "Platform", "Special", "RoomBounds", "Turret", "Block", "Rope", "Water", "BreakableWall", "Staircase", "Spike", "Elevator" };
    ...
    {
        if (fill == "#00ffff" || fill == "#00FFFF") return ObjectType::Floor;
        if (fill == "#ffff00" || fill == "#FFFF00") return ObjectType::Platform;
        if (fill == "#ff0000" || fill == "#FF0000") return ObjectType::Special;
        if (fill == "#ffffff" || fill == "#FFFFFF") return ObjectType::RoomBounds;
        if (fill == "#ff6600" || fill == "#FF6600") return ObjectType::Turret;
        if (fill == "#cc00cc" || fill == "#CC00CC") return ObjectType::Block;
        if (fill == "#00cc00" || fill == "#00CC00") return ObjectType::Rope;
        if (fill == "#0044cc" || fill == "#0044CC") return ObjectType::Water;
        if (fill == "#884400") return ObjectType::BreakableWall;
        if (fill == "#aa6600" || fill == "#AA6600") return ObjectType::Staircase;
        if (fill == "#cc2222" || fill == "#CC2222") return ObjectType::Spike;
        if (fill == "#4488cc" || fill == "#4488CC") return ObjectType::Elevator;
...
void LevelEditor::HandleCamera()
{
    ...
    if (!ImGui::GetIO().WantCaptureKeyboard)
    {
        ...
        if (input.KeyJustPressed(CS230::Input::Keys::Right)) editorCamPos.x += stepX;
        if (input.KeyJustPressed(CS230::Input::Keys::Left))  editorCamPos.x -= stepX;
        if (input.KeyJustPressed(CS230::Input::Keys::Up))    editorCamPos.y += stepY;
        if (input.KeyJustPressed(CS230::Input::Keys::Down))  editorCamPos.y -= stepY;
    }
...
void LevelEditor::DrawGrid(CS200::IRenderer2D& renderer, Math::ivec2 winSize)
{
    ...
    const Math::vec2 br = ScreenToWorld({ static_cast<double>(winSize.x),
                                          static_cast<double>(winSize.y) });
...
void LevelEditor::UpdateObjectCreation()
{
    ...
    if (currentMode == EditorMode::Rect)
    {
        ...
        if (isDrawingRect && input.MouseButtonJustReleased(CS230::Input::MouseButton::Left))
        {
            ...
            if (x1 - x0 > 4.0 && y1 - y0 > 4.0)
            {
                ...
                obj.vertices = { {x0,y0}, {x1,y0}, {x1,y1}, {x0,y1} };
    ...
    if (currentMode == EditorMode::Polygon)
    {
        ...
        if (input.KeyJustPressed(CS230::Input::Keys::Enter) &&
            polyInProgress.size() >= 3)
        {
            PushUndo();
            EditorObject obj;
            obj.isRect   = false;
...
void LevelEditor::UpdateSelectionAndEdit()
{
    ...
    if (currentMode == EditorMode::View)
    {
        if (!imguiMouse && input.MouseButtonJustPressed(CS230::Input::MouseButton::Left))
        {
            ...
            if (selectedIndex >= 0 &&
                PointInPolygon(objects[selectedIndex].vertices, mouseWorld))
            {
                PushUndo();
        ...
        if (isDraggingObject && input.MouseButtonDown(CS230::Input::MouseButton::Left) &&
            selectedIndex >= 0)
        {
        ...
        selectedIndex >= 0 && selectedIndex < static_cast<int>(objects.size()))
    {
        ...
        if (draggedVertexIndex >= 0 &&
            input.MouseButtonDown(CS230::Input::MouseButton::Left))
        {
...
void LevelEditor::DrawDashedLine(CS200::IRenderer2D& renderer,
                                  Math::vec2 from, Math::vec2 to,
...
void LevelEditor::DrawObjects(CS200::IRenderer2D& renderer)
{
    ...
    if (selectedIndex >= 0 && selectedIndex < static_cast<int>(objects.size()))
    {
        ...
        if (currentMode == EditorMode::VertexEdit)
        {
            for (int i = 0; i < static_cast<int>(verts.size()); ++i)
            {
                ...
                const auto t = Math::TranslationMatrix(verts[i])
                             * Math::ScaleMatrix(HANDLE_R * 2.0);
    ...
    if (currentMode == EditorMode::Rect && isDrawingRect)
    {
        ...
        for (const Math::vec2 c : { Math::vec2{x0,y0}, {x1,y0}, {x1,y1}, {x0,y1} })
        {
    ...
    if (currentMode == EditorMode::Polygon && !polyInProgress.empty())
    {
        ...
        const auto t = Math::TranslationMatrix(polyInProgress.front())
                     * Math::ScaleMatrix(VERT_R * 2.2);
...
void LevelEditor::LoadSVG()
{
    ...
    for (; it != end; ++it)
    {
        ...
        else if (tagName == "rect" || tagName == "RECT")
        {
            ...
            try
            {
                if (std::regex_search(tag, m, rRectX)) rx = std::stod(m[1].str());
                if (std::regex_search(tag, m, rRectY)) ry = std::stod(m[1].str());
                if (std::regex_search(tag, m, rRectW)) rw = std::stod(m[1].str());
                if (std::regex_search(tag, m, rRectH)) rh = std::stod(m[1].str());
            }
            catch (...) { continue; }
    ...
    // Load spawn position from orange circle element
    {
        ...
        for (; cit != std::sregex_iterator{}; ++cit)
        {
            ...
            if (std::regex_search(ctag, cm, rCX))
                try { s_spawnPos.x =  std::stod(cm[1].str()); } catch (...) {}
            if (std::regex_search(ctag, cm, rCY))
                try { s_spawnPos.y = -std::stod(cm[1].str()); } catch (...) {}
            break;
        }
    }
...
void LevelEditor::UpdateSaveLoad()
{
    ...
    if (ctrl && input.KeyJustPressed(CS230::Input::Keys::D))
    {
        if (selectedIndex >= 0 && selectedIndex < static_cast<int>(objects.size()))
        {
            ...
            for (auto& v : copy.vertices) { v.x += gridSize; v.y -= gridSize; }
...
void LevelEditor::Draw()
{
    ...
    const Math::TransformationMatrix vp =
        CS200::build_ndc_matrix(win) * GetViewMatrix();
    ...
    DrawGrid(r, win);
    DrawObjects(r);
    if (scriptEditorMode) DrawTriggers(r);
...
void LevelEditor::DrawImGui()
{
    ...
    ImGui::Begin("##editor_hud", nullptr,
    ...
    // Script Editor toggle button
    {
        ...
        if (wasScript) ImGui::PushStyleColor(ImGuiCol_Button, { 0.6f, 0.3f, 0.8f, 1.0f });
        if (ImGui::Button(scriptEditorMode ? "Script Ed [ON]" : "Script Ed", { 265.0f, 0.0f }))
        {
            scriptEditorMode = !scriptEditorMode;
            if (!scriptEditorMode) placingTrigger = false;
        }
        if (wasScript) ImGui::PopStyleColor();
    }
    ...
    auto ModeBtn = [&](const char* label, EditorMode mode, bool enabled = true)
    {
        ...
        if (active) ImGui::PushStyleColor(ImGuiCol_Button, { 0.2f, 0.6f, 0.2f, 1.0f });
    ...
    ModeBtn("View[V]",  EditorMode::View);    ImGui::SameLine();
    ModeBtn("Rect[R]",  EditorMode::Rect);    ImGui::SameLine();
    ModeBtn("Poly[P]",  EditorMode::Polygon); ImGui::SameLine();
    ModeBtn("Vtx[E]",   EditorMode::VertexEdit,
            selectedIndex >= 0 && selectedIndex < static_cast<int>(objects.size()));
    ...
    if (selectedIndex >= 0 && selectedIndex < static_cast<int>(objects.size()))
    {
        ...
        ImGui::TextColored({ 1.0f, 1.0f, 0.4f, 1.0f }, "Sel #%d (%s)",
                           selectedIndex, selObj.isRect ? "Rect" : "Poly");
        ...
        {
            ImGui::Separator();
            ImGui::TextColored({ 1.0f, 0.4f, 0.0f, 1.0f }, "Turret ID: %s",
        ...
        {
            ImGui::Separator();
            ImGui::TextColored({ 0.27f, 0.53f, 0.8f, 1.0f }, "Elevator ID: %s",
                               selObj.id.empty() ? "(none)" : selObj.id.c_str());
            ...
            float elevPeriod = 4.0f;
            {
                ...
                if (p1 != std::string::npos)
                    try { elevDist = std::stof(sid.substr(p1 + 1, p2 - p1 - 1)); } catch (...) {}
                if (p2 != std::string::npos)
                    try { elevPeriod = std::stof(sid.substr(p2 + 1)); } catch (...) {}
            }
        ...
        if (ImGui::TreeNode("Vertices"))
        {
            for (int vi = 0; vi < static_cast<int>(selObj.vertices.size()); ++vi)
                ImGui::Text("[%d] (%.0f, %.0f)", vi,
                            selObj.vertices[vi].x, selObj.vertices[vi].y);
            ImGui::TreePop();
        }
    ...
    // ---- Object list ----
    ImGui::Separator();
    ImGui::Text("Objects: %d   Undo: %d",
                static_cast<int>(objects.size()), static_cast<int>(undoStack.size()));
    ...
    for (int i = 0; i < static_cast<int>(objects.size()); ++i)
    {
        ...
        snprintf(label, sizeof(label), "#%d %s (%s)",
                 i, objects[i].isRect ? "Rect" : "Poly",
                 TypeNames[static_cast<int>(objects[i].type)]);
    ...
    // ---- Save / Load / Play ----
    if (ImGui::Button("Save[Ctrl+S]", { 130.0f, 0.0f })) SaveSVG();
    ImGui::SameLine();
    if (ImGui::Button("Load[Ctrl+O]", { 130.0f, 0.0f })) LoadSVG();
    ...
    ImGui::SameLine();
    ImGui::SetNextItemWidth(130.0f);
    if (ImGui::InputFloat("Y##spawny", &spArr[1], 40.0f, 200.0f, "%.0f")) s_spawnPos.y = static_cast<double>(spArr[1]);

    // Status message (disappears after 2 s)
    if (statusTimer > 0.0)
    {
        const bool   isError = statusMessage.find("FAIL") != std::string::npos;
        const ImVec4 col     = isError ? ImVec4{1.f, 0.4f, 0.4f, 1.f}
...
void LevelEditor::SaveTriggers()
{
    ...
    std::ofstream f(path);
    if (!f.is_open()) { statusMessage = "Trigger save FAILED"; statusTimer = 2.0; return; }
...
void LevelEditor::DrawTriggers(CS200::IRenderer2D& r)
{
    ...
    for (int i = 0; i < static_cast<int>(triggers.size()); ++i)
    {
        ...
        const auto mat = Math::TranslationMatrix(t.pos)
                       * Math::ScaleMatrix(t.radius);
    ...
    if (placingTrigger)
    {
        ...
        const double     r_   = (selectedTrigger >= 0 && selectedTrigger < static_cast<int>(triggers.size()))
                                 ? triggers[selectedTrigger].radius : 120.0;
    ...
    for (int i = 0; i < static_cast<int>(editEvents.size()); ++i)
    {
        ...
        if (e.type != EventType::CameraPan && e.type != EventType::MovePlayer) continue;
...
void LevelEditor::UpdateTriggerEditing()
{
        ...
        input.MouseButtonJustPressed(CS230::Input::MouseButton::Left))
    {
        ...
        // Sync ID buf
        std::copy(t.id.begin(), t.id.begin() + std::min(t.id.size(), sizeof(triggerIdBuf) - 1),
                  triggerIdBuf);
        triggerIdBuf[t.id.size()] = '\0';
        std::copy(t.scriptFile.begin(), t.scriptFile.begin() + std::min(t.scriptFile.size(), sizeof(scriptNameBuf) - 1),
                  scriptNameBuf);
    ...
    if (!placingTrigger && selectedEvent >= 0 &&
        selectedEvent < static_cast<int>(editEvents.size()) && !imguiMouse)
    {
        ...
        else if (selEv.type == EventType::CameraPan && drawingCamRect &&
                 input.MouseButtonJustPressed(CS230::Input::MouseButton::Left))
        {
        ...
        if (selEv.type == EventType::MovePlayer &&
            input.MouseButtonJustPressed(CS230::Input::MouseButton::Left))
        {
    ...
    if (!placingTrigger && drawingCamRect && selectedEvent >= 0 &&
        selectedEvent < static_cast<int>(editEvents.size()) &&
        editEvents[selectedEvent].type == EventType::CameraPan &&
        !imguiMouse && input.MouseButtonJustPressed(CS230::Input::MouseButton::Left))
    {
        camRectDragStart  = SnapToGrid(mouseWorld, gridSize);
        camRectInProgress = true;
    }
    ...
    if (!ImGui::GetIO().WantCaptureKeyboard)
    {
        ...
        if (curT && !prevT) placingTrigger = !placingTrigger;
        prevT = curT;
    }
}
...
void LevelEditor::DrawScriptEditorPanel()
{
    ...
    ImGui::Begin("##script_panel", nullptr,
    ...
    if (selectedTrigger >= 0 && selectedTrigger < static_cast<int>(triggers.size()))
    {
        ...
        if (ImGui::InputFloat("X##tpx", &pos[0], 40.0f, 200.0f, "%.0f")) t.pos.x = static_cast<double>(pos[0]);
        ImGui::SameLine();
        ImGui::SetNextItemWidth(120.0f);
        if (ImGui::InputFloat("Y##tpy", &pos[1], 40.0f, 200.0f, "%.0f")) t.pos.y = static_cast<double>(pos[1]);
    ...
    ImGui::Separator();
    if (ImGui::Button("Save Triggers##ts", { 136.0f, 0.0f })) SaveTriggers();
    ImGui::SameLine();
    if (ImGui::Button("Load Triggers##tl", { 136.0f, 0.0f })) LoadTriggers();
...
void LevelEditor::DrawTimelinePanel()
{
    ...
    ImGui::TextColored({ 0.8f, 0.4f, 1.0f, 1.0f }, "Timeline: %s -> %s.txt",
    ...
    // View range + add event controls
    {
        ...
        if (ImGui::Button("+ Event"))
        {
            ...
            ev.duration = (ev.type == EventType::Lock   ||
                           ...
                           ev.type == EventType::MovePlayer) ? 0.0 : 1.0;
    ...
    for (int tr = 0; tr < numTracks; ++tr)
    {
        ...
        dl->AddText({ canvasX - LABEL_W + 4.0f, ty + 4.0f },
                    IM_COL32(180, 180, 180, 255), trackLabels[tr]);
        dl->AddLine({ canvasX - LABEL_W, ty + trackH }, { canvasX + canvasW, ty + trackH },
    ...
    {
        const auto& e = editEvents[i];
        if (e.time > timelineViewEnd) continue;
        ...
        const ImU32  col  = IM_COL32(static_cast<int>(cv4.x * 255),
                                      static_cast<int>(cv4.y * 255),
        ...
        dl->AddRect({ ex, ey }, { ex + ew, ey + trackH - 4.0f }, selCol, 3.0f,
                    0, (i == selectedEvent) ? 2.0f : 1.0f);
    ...
    if (inCanvas && ImGui::IsMouseClicked(ImGuiMouseButton_Left))
    {
        ...
        {
            const auto& e = editEvents[i];
            if (EventTrack(e.type) == clickTrack &&
                clickTime >= e.time && clickTime <= e.time + std::max(e.duration, 0.1))
            {
                selectedEvent = i;
                break;
            }
    ...
    if (selectedEvent >= 0 && selectedEvent < static_cast<int>(editEvents.size()))
    {
        ...
        if (e.type == EventType::CameraPan)
        {
            ...
            if (ImGui::InputFloat("CX##ecp", &pp[0], 40.0f, 200.0f, "%.0f")) e.position.x = static_cast<double>(pp[0]);
            ImGui::SameLine();
            ImGui::SetNextItemWidth(90.0f);
            if (ImGui::InputFloat("CY##ecpy", &pp[1], 40.0f, 200.0f, "%.0f")) e.position.y = static_cast<double>(pp[1]);
            ImGui::SameLine();

            // viewSize display / edit
            float vs[2] = { static_cast<float>(e.viewSize.x), static_cast<float>(e.viewSize.y) };
            ImGui::SetNextItemWidth(80.0f);
            if (ImGui::InputFloat("VW##evsx", &vs[0], 40.0f, 200.0f, "%.0f")) e.viewSize.x = std::clamp(static_cast<double>(vs[0]), 0.0, 5000.0);
            ImGui::SameLine();
            ImGui::SetNextItemWidth(80.0f);
            if (ImGui::InputFloat("VH##evsy", &vs[1], 40.0f, 200.0f, "%.0f")) e.viewSize.y = std::clamp(static_cast<double>(vs[1]), 0.0, 5000.0);
            ImGui::SameLine();
        }
        ...
        if (e.type == EventType::MovePlayer)
        {
            float pp[2] = { static_cast<float>(e.position.x), static_cast<float>(e.position.y) };
            ImGui::SetNextItemWidth(90.0f);
            if (ImGui::InputFloat("PX##empx", &pp[0], 40.0f, 200.0f, "%.0f")) e.position.x = static_cast<double>(pp[0]);
            ImGui::SameLine();
            ImGui::SetNextItemWidth(90.0f);
            if (ImGui::InputFloat("PY##empy", &pp[1], 40.0f, 200.0f, "%.0f")) e.position.y = static_cast<double>(pp[1]);
            ImGui::SameLine();
        }

        if (e.type == EventType::Text)
        {
            std::copy(e.text.begin(),
                      e.text.begin() + std::min(e.text.size(), sizeof(eventTextBuf) - 1),
                      eventTextBuf);
        ...
        if (e.type == EventType::Text)
        {
            ...
            ImGui::SetNextItemWidth(130.0f);
            if (ImGui::InputFloat("X##etpx", &px, 0.05f, 0.2f, "%.2f")) e.position.x = std::clamp(static_cast<double>(px), 0.0, 1.0);
            ImGui::SameLine();
            ImGui::SetNextItemWidth(130.0f);
            if (ImGui::InputFloat("Y##etpy", &py, 0.05f, 0.2f, "%.2f")) e.position.y = std::clamp(static_cast<double>(py), 0.0, 1.0);

            ImGui::TextDisabled("Presets:");
            ImGui::SameLine();
            if (ImGui::SmallButton("Bot##ep"))  e.position = { 0.5, 0.12 };
            ImGui::SameLine();
            if (ImGui::SmallButton("Top##ep"))  e.position = { 0.5, 0.88 };
            ImGui::SameLine();
            if (ImGui::SmallButton("L##ep"))    e.position = { 0.15, 0.5 };
            ImGui::SameLine();
            if (ImGui::SmallButton("R##ep"))    e.position = { 0.85, 0.5 };
            ImGui::SameLine();
            if (ImGui::SmallButton("Ctr##ep"))  e.position = { 0.5, 0.5 };

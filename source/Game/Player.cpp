...
namespace
{
    ...
    // IK Result Structure
    struct TwoBone { Math::vec2 mid; Math::vec2 end; };
...
TwoBone SolveTwoBoneStraight(Math::vec2 start, double L1, double L2, Math::vec2 target, bool keepLength) {
        ...
        if (keepLength) currentLen = totalLen;
...
Player::Player(Math::vec2 start_pos)
    : CS230::GameObject(start_pos), startPosition(start_pos), previousPosition(start_pos), shieldComponent(nullptr), isJumping(true), velocityY(0.0), faceRight(true)
{
...
void Player::Update(double dt)
{
    ...
    if (jumpBufferTimer > 0.0) jumpBufferTimer -= dt;
    if (coyoteTimer > 0.0) coyoteTimer -= dt;

    HandleInput(dt);
    dashComponent.UpdateTimers(dt);

    bool applyGravity = true;
    if (dashComponent.IsDashing() && dashComponent.disableGravityOnDash) applyGravity = false;

    if (applyGravity) velocityY -= gravity * dt;

    double finalVelX = GetVelocity().x;
    if (dashComponent.IsDashing()) finalVelX = dashComponent.GetDashVelocityX();
...
void Player::UpdateProceduralAnimation(double dt)
{
    ...
    float nearMulSpan = 1.18f; float farMulSpan  = 0.72f;
    float nearLenMul  = 1.00f; float farLenMul   = 0.92f;
    float nearYOffset = -2.0f; float farYOffset  = +4.0f;
    ...
    if (!isGrounded) {
        ...
        double strideBase = 16.0; double stepLiftBase = 8.0;
...
{
    if (other_object_type == GameObjectTypes::Floor) return true;
    if (other_object_type == GameObjectTypes::Sign) return true;
    if (other_object_type == GameObjectTypes::Bonfire) return true;
    if (other_object_type == GameObjectTypes::Door) return true;
...
void Player::ResolveCollision(GameObject* other_object)
{
    ...
    if (other_object->Type() == GameObjectTypes::Floor)
    {
        ...
        if (!horizontal_overlap || !vertical_overlap) return;
        ...
        else
        {
            ...
            if (overlap_left < min_overlap) { min_overlap = overlap_left; axis = 2; }
            if (overlap_right < min_overlap) { min_overlap = overlap_right; axis = 3; }
...
void Player::Draw(const Math::TransformationMatrix& camera_matrix)
{
    ...
    if (skeleton)
    {
        ...
        auto AddLeftLimbs = [&]() {
            drawOrder.push_back("L_Thigh"); drawOrder.push_back("L_Calf");
            drawOrder.push_back("L_Clavicle"); drawOrder.push_back("L_Arm_Up"); drawOrder.push_back("L_Arm_Low");
        };
        auto AddRightLimbs = [&]() {
            drawOrder.push_back("R_Thigh"); drawOrder.push_back("R_Calf");
            drawOrder.push_back("R_Clavicle"); drawOrder.push_back("R_Arm_Up"); drawOrder.push_back("R_Arm_Low");
        };
        auto AddBody = [&]() {
            drawOrder.push_back("Hips"); 
            drawOrder.push_back("SpineLower"); drawOrder.push_back("SpineUpper"); 
            drawOrder.push_back("Neck"); drawOrder.push_back("Head");

...
bool SimpleBossStar::IsPlayerInRoom() const
{
    ...
    return p.x >= roomBounds.Left() &&
           p.x <= roomBounds.Right() &&
           p.y >= roomBounds.Bottom() &&
           p.y <= roomBounds.Top();
}
...
void SimpleBossStar::StartWarning()
{
    ...
    if (player != nullptr)
    {
        attackDir = SafeNormalize(player->GetPosition() - GetPosition(), attackDir);
    }
}
...
void SimpleBossStar::UpdateState(double dt)
{
    ...
    case State::TrackingLaser:
    {
        ...
        if (DistanceToSegmentSquared(player->GetPosition(), start, end) <=
            TrackingDamageRadius * TrackingDamageRadius)
        {
...
void SimpleBossStar::DrawBossBody()
{
    ...
    const auto aura =
        Math::TranslationMatrix(GetPosition()) *
        Math::ScaleMatrix({ 150.0 * pulse, 150.0 * pulse });

    const auto body =
        Math::TranslationMatrix(GetPosition()) *
        Math::ScaleMatrix({ 70.0 * pulse, 70.0 * pulse });
...
void SimpleBossStar::DrawShots()
{
    ...
    for (const Shot& shot : shots)
    {
        ...
        for (int i = 0; i < n; ++i)
        {
            ...
            const auto mat =
                Math::TranslationMatrix(shot.trail[i]) *
                Math::ScaleMatrix({ radius * 2.0, radius * 2.0 });
        ...
        const auto outer =
            Math::TranslationMatrix(shot.pos) *
            Math::ScaleMatrix({ coreSize, coreSize });

        const auto inner =
            Math::TranslationMatrix(shot.pos) *
            Math::ScaleMatrix({ 8.0, 8.0 });

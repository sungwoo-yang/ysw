...
void AnimationEditor::ApplyKeyframe(double time)
{
    for (auto& [name, track] : currentAnim.tracks)
    {
        ...
        if (time <= keys.front().time)
            angle = keys.front().value;
        else if (time >= keys.back().time)
            angle = keys.back().value;
        else
        {
...
void AnimationEditor::DrawImGui()
{
    ...
    if (ImGui::Begin("Animation Editor"))
    {
        ...
        {
            std::vector<std::string> boneList = { "Hips",   "SpineLower", "SpineUpper", "Neck",      "Head",       "L_Thigh",  "L_Calf",   "R_Thigh",
                                                  "R_Calf", "L_Clavicle", "L_Arm_Up",   "L_Arm_Low", "R_Clavicle", "R_Arm_Up", "R_Arm_Low" };
...
void AnimationEditor::LoadAnimation(const std::filesystem::path& filename)
{
    ...
    std::copy(name.begin(), name.begin() + static_cast<std::string::difference_type>(len), animNameBuffer);

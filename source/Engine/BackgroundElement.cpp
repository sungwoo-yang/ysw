...
namespace CS230
{
    BackgroundElement::BackgroundElement(Math::vec2 pos, const std::string& texturePath) : GameObject(pos), texturePtr(nullptr)
    {
    ...
    void BackgroundElement::Draw(const Math::TransformationMatrix& camera_matrix)
    {
        if (texturePtr)
        {
            ...
            Math::TransformationMatrix worldMatrix = Math::TranslationMatrix(GetPosition()) * Math::ScaleMatrix(GetScale()) * Math::TranslationMatrix(centerOffset);
